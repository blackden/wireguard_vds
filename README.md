...
### Новое в 2.0
- Общая библиотека `lib/common.sh` с повторно используемыми функциями (`require_root`, логи, управление сервисом WireGuard).
- Усилены проверки окружения и прав пользователя во всех сценариях.
- Команды управления сервисом и интерфейсом приведены к единообразным функциям.
- Скрипты проходят проверку `shellcheck`.

## Требования
- Ubuntu Server 18.04 и новее.
- Доступ с правами `root` (все сценарии проверяют права и завершаются, если запущены не от суперпользователя).
